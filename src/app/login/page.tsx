...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      // 1. Verify that this email actually exists in the profiles database
      const existingProfile = await syncService.getUserProfile(cleanedEmail);
      if (!existingProfile) {
        setError('You do not have an account. Please register/sign up first!');
        setLoading(false);
        return;
      }
      ...
      // 2. Perform native Supabase Auth Sign In
      ...
      // 2. Fetch corresponding app profile linked to the Auth UUID
      const profile = await syncService.getUserProfileById(data.user.id);
      ...
      // Success - Save session token and update store state
      sessionStorage.setItem('smart-study-session', 'active');
      setUser({ 
        id: data.user.id,
        name: profile.name, // Registered Full Name is used as Username!
        email: profile.email,
